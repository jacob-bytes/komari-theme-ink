...
<<<<<<< HEAD
| v0.5.x | 详情页去冗余单层呈现 + 图形语义强化 + 移除交互式地球（性能/维护成本收敛）+ 卡片网格虚拟滚动 + 配置项梳理 + echarts 死组件裁剪 + Header 数据新鲜度绑定真实数据到达时间 |
=======
| v0.5.x | 详情页去冗余单层呈现 + 图形语义强化 + 移除交互式地球（性能/维护成本收敛）+ 卡片网格虚拟滚动 + 配置项梳理 + echarts 死组件裁剪 |
>>>>>>> origin/v0/github-dfc5dc3f
